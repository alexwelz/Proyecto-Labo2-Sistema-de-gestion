...
//TODO: Login empleados (NO SE PUEDEN HACER MAS DE 3 INTENTOS FALLIDOS)
void menuEmpleado()
{
    int legajo;
    int password;
                ...
                do
                {
                    ...
                    if (encontro == true && cancelado == false)
                    {
                        ...
                        if (stockInsuficiente)
                        {
                            ...
                            ubicar(27, 10);
                            rlutil::setColor(rlutil::RED);
                            cout << "Cantidad de producto no valida";
                            rlutil::setColor(rlutil::CYAN);
                            presioneUnaTecla();
                            limpiarConsola();
                            return menuEmpleadoIngreso(legajo, categoria);
...
void menuInformes()
{
    ...
    do
    {
        limpiarConsola();//Utilizado para opcion --> Seleccionar otro empleado 
        rlutil::setBackgroundColor(rlutil::BLACK);
        rlutil::setColor(rlutil::WHITE);
        rlutil::hidecursor();
        showItem("                   Menu de informes", 30, 9, 0);
        showItem("======================================================= ", 30, 10, 0);
        showItem("- Informes por empleado       ", 38, 11, y == Opcion1);
        showItem("- Informes globales           ", 38, 12, y == Opcion2);
        showItem("======================================================= ", 30, 13, 0);
        showItemEgreso("- Volver          ", 38, 14, y == Opcion3);
        ...
        case 1: // ENTER
            ...
            switch (y)
            {
                //TODO: ACA SE MODIFICAN LOS INFORMES POR EMPLEADO
            case 0:
            {
                op = 0;
                do
                {
                    ...
                    for (int x = 0; x < empArc.getCantidad(); x++)
                    {
                        ...
                        if (emp.getLegajo() == idEmp && emp.getCategoria() == 999)
                        {
                            rlutil::setColor(rlutil::RED);
                            limpiarConsola();
                            ubicar(27, 10);
                            cout << "El ADMIN no realiza pedidos!" << endl;
                            ubicar(27, 12);
                            presioneUnaTecla();
                            limpiarConsola();
                            return menuInformes();
                ...
                do
                {
                    ...
                    showItem("         - Seleccionar otro empleado                   ", 30, 22, y == Opcion5);
                    showItem("=======================================================", 30, 23, 0);
                    showItem("         - Volver menu principal                       ", 30, 24, y == Opcion6);
            ...
            //TODO: ACA SE MODIFICAN LOS INFORMES GLOBALES
            case 1:
            {
                //op = 0;
                op = 1;
                ...
                do
                {
                    showItem("                   Informes Globales", 30, 9, 0);
                    showItem("======================================================= ", 30, 10, 0);
                    showItemIngreso("- Productos cargados                    ", 38, 11, y == Opcion1);
                    showItemIngreso("- Pedidos cargados          ", 38, 12, y == Opcion2);
                    showItem("======================================================= ", 30, 13, 0);
                    showItemIngreso("- Volver          ", 38, 14, y == Opcion3);
                    ...
                    case 1: // ENTER
                        ...
                        case 0:
                        {
                            ...
                            Pedido* ped;
                            PedidosArchivo pedArc;
                            int cantEmp = empArc.getCantidad();
                            int cantPed = pedArc.getCantidad();
                            int acuCargados = 0;
                            int acuPendientes = 0;
                            ped = new Pedido[cantPed];
                            emp = new Empleado[cantEmp];

                            int ejex = 30;
                            int ejey = 10;

                            for (int x = 0; x < cantEmp; x++)
                            {
                                emp[x] = empArc.leer(x);
                            }
                            for (int x = 0; x < cantPed; x++)
                            ...
                            cout << setw(10) << "CARGADOS";
                            cout << setw(11) << "PENDIENTES";
                            cout << setw(5) << "TOTAL";
                            cout << endl;
                            ubicar(ejex, ++ejey);
                            cout << "----------------------------------------------";
                            ...
                            for (int x = 0; x < cantEmp; x++)
                            {
                                ...
                                {
                                    if (emp[x].getLegajo() == ped[i].getLegajo() && ped[i].getEstadoPedido() == true && strcmp(ped[i].getTipoPedido(), "c") != 0)
                                    {
                                        acuCargados += ped[i].getCantidadProducto();
                                    ...
                                    else
                                    {
                                        if (emp[x].getLegajo() == ped[i].getLegajo() && ped[i].getEstadoPedido() != true && strcmp(ped[i].getTipoPedido(), "c") != 0)
                                        {
                                            acuPendientes += ped[i].getCantidadProducto();
                                        }
                                    }
                                }
                                if (emp[x].getCategoria() != 999)
                                {
                                    ubicar(ejex, ++ejey);
                                    cout << setw(10) << emp[x].getNombre() << setw(10) << emp[x].getApellido();
                                    cout << setw(10) << acuCargados;
                                    cout << setw(11) << acuPendientes;
                                    cout << setw(6) << acuCargados + acuPendientes << endl;
                                }
                        ...
                        case 1:
                        {
                            ...
                            Producto* prod;
                            ProductosArchivo prodArc;
                            int cantProd = prodArc.getCantidad();
                            int cantEmp = empArc.getCantidad();
                            int cantPed = pedArc.getCantidad();
                            int cont = 0;
                            ped = new Pedido[cantPed];
                            emp = new Empleado[cantEmp];
                            prod = new Producto[cantProd];
                            ...
                            for (int x = 0; x < cantProd; x++)
                            {
                                prod[x] = prodArc.leer(x);
                            }
                            ...
                            for (int x = 0; x < cantEmp; x++)
                            {
                                ...
                                for (int i = 0; i < cantPed; i++)
                                {
                                    if (emp[x].getLegajo() == ped[i].getLegajo()
                                        && ped[i].getEstadoPedido() == true
                                        && strcmp(ped[i].getTipoPedido(), "c") != 0
                                        && ped[i].getIdPedido() != idPedAnt)
                                    {
                                        idPedAnt = ped[i].getIdPedido();
                                        cont++;
                                    }
                                }
                                ...
                                {
                                    ubicar(ejex, ++ejey);
                                    cout << setw(10) << emp[x].getLegajo() << setw(10) << emp[x].getNombre() << setw(10) << emp[x].getApellido() << setw(9) << cont;
                                    if (emp[x].getCategoria() == 1)
                                    {
                                        cout << " INGRESO" << endl;
                                    }
                                    else
                                    {
                                        if (emp[x].getCategoria() == 2)
                                        {
                                            cout << " EGRESO" << endl;
                                        }
                                    }
